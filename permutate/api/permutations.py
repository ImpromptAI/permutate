...
def get_llm_permutations(
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"message": "Failed to get LLM Permutations"}
        )
...
def get_tool_selector_permutations(
    ...
    try:
        # Read data from a JSON file
        with open(
            "permutate/templates/tool_selector_permutations.json", "r"
        ) as json_file:
            data = json.load(json_file)
            conditions = []
            # Apply filters based on query parameters
        ...
        return JSONResponse(
            status_code=500,
            content={"message": "Failed to get Tool Selector Permutations"},
        )
